...
public class PlanillaPanel extends Panel {
    ...
    public PlanillaPanel() {
        ...
        add(new Label("año", new LocalDateTime().year().getAsShortText()));
        add(new Label("lugar", "Paseo Colón"));
        add(new Label("nombreLocal", ""));
        add(new Label("nombreVisitante", ""));

        List<Jugador> local = new ArrayList<Jugador>();
        List<Jugador> visitante = new ArrayList<Jugador>();
        add(new ListView<Jugador>("fichasJugadoresLocales", local) {

            @Override
            protected void populateItem(ListItem<Jugador> item) {
                item.add(new Label("fichasLocales", ""));
            }

        });

        add(new ListView<Jugador>("jugadoresLocales", local) {

            @Override
            protected void populateItem(ListItem<Jugador> item) {
                item.add(new Label("locales", ""));
            }
        });

        add(new ListView<Jugador>("numerosJugadoresLocales", local) {

            @Override
            protected void populateItem(ListItem<Jugador> item) {
                item.add(new Label("numerosLocales", ""));
            }
        });
        
        add(new ListView<Jugador>("fichasJugadoresVisitantes", visitante) {

            @Override
            protected void populateItem(ListItem<Jugador> item) {
                item.add(new Label("fichasVisitantes", ""));
            }
        });
        
        add(new ListView<Jugador>("jugadoresVisitantes", visitante) {

            @Override
            protected void populateItem(ListItem<Jugador> item) {
                item.add(new Label("visitantes", ""));
            }
        });
        
        add(new ListView<Jugador>("numerosJugadoresVisitantes", visitante) {

            @Override
            protected void populateItem(ListItem<Jugador> item) {
                item.add(new Label("numerosVisitantes", ""));
            }
        });
    }
